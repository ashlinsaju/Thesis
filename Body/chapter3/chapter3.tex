...
\end{enumerate}
For simplicity and ease of programming we have selected one of the most popular processor which is Arduino Ethernet board which has ATmega328 microcontroller. It is an open source physical computing platform based on simple input/output (IO) board and a development environment that implements the processing language\cite{Banzi2008}. It is fairly easy and simple to use with any external devices such as sensors or actuators and is widely used by researchers. There are different features which make arduino popular and can be listed as \cite{Banzi2008}:
